...
export class StripeService {
  ...
  async createPaymentIntent(amountInDollars = 49.99, currency: string = 'usd', email?:string) {
  ...
  async handleSuccessfulPayment(intent: Stripe.PaymentIntent): Promise<void> {
      const paymentIntent = await this.stripe.paymentIntents.retrieve(intent.id, {
        expand: ['charges'],
      }) as unknown as Stripe.PaymentIntent & { charges: { data: Stripe.Charge[] } };
      ...
      const membership = user.membership ?? this.membershipRepository.create({ user });
  ...
  async handleFailedPayment(intent: Stripe.PaymentIntent): Promise<void> {
      ...
      if (!email) {
        console.warn('No se pudo determinar el correo electrónico del usuario en pago fallido');
        return;
      }
